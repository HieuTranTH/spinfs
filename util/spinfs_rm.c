...
void print_usage()
{
        ...
        fprintf(stderr, "Format: spinfs_rm path\n");
        fprintf(stderr, "\n");
}
...
struct spinfs_raw_inode *update_deleted_inode(struct spinfs_raw_inode *s)
{
...
{
        centerTitleText("REMOVING a FILE", TITLE_TEXT_WIDTH);
        ...
        inode = update_deleted_inode(inode);
        ...
        free(inode);
        free(parent);
        centerTitleText("DONE REMOVING a FILE", TITLE_TEXT_WIDTH);
        return 0;
}
